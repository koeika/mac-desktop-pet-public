...
```bash
curl -X POST http://127.0.0.1:4789/v1/progress \
  -H 'content-type: application/json' \
  -d '{"source":"codex","stage":"验证","message":"正在运行测试","progress":80}'
```
...
或把模板里的 `<path-to-mac-desktop-pet>` 替换成你自己的本地 clone 路径。不要把真实本机路径提交到公开仓库。

## 词汇反馈规则

- `认识啦`：标记为已学，降低后续出现频率。
- `再解释`：标记为待加强，展示释义/例句/提示，提高后续出现频率。
- `先跳过`：降低后续出现频率。
